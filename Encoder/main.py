"""
Graph Encoder Module for Personalized Influence Maximization.

This module implements the encoder part of the architecture:
- Graph Transformer (DAT: Diffusion-Aware Transformer) for graph representation learning
- Laplacian positional encoding for structural information
- Node and edge feature engineering

The encoder transforms raw graph data into learned embeddings that capture
structural and semantic information, which are then used by the decoder
for decision making.
"""
...
def laplacian_positional_encoding(g, pos_enc_dim):
    ...
    Laplacian positional encoding captures structural information about node positions
    in the graph by computing eigenvectors of the graph Laplacian matrix.
    This helps the model understand graph topology and node relationships.
    
    Algorithm:
    1. Compute normalized symmetric Laplacian matrix
    2. Compute k smallest eigenvalues and eigenvectors
    3. Use eigenvectors as positional encodings (excluding first eigenvector)
    ...
    Returns:
        Graph object with lap_pos_enc attribute added
    """
    weight = []
    ...
    # Use eigenvectors (excluding first one, which is constant)
...
def MyGT_train(g, net):
    """
    Train Graph Transformer to generate node and edge embeddings.
    
    This function:
    1. Adds Laplacian positional encoding if enabled
    2. Initializes Graph Transformer model
    3. Generates node embeddings (h) and edge embeddings (e)
    
    Note: Model parameters are frozen (requires_grad=False) as this is
    typically used for inference with a pre-trained encoder.
    
    Args:
        g: Graph data object
        net: Network configuration dictionary
    ...
    # Freeze model parameters (typically used for inference)
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',
                                                     ...
                                                     patience=15,
                                                     verbose=True)
    # Apply random sign flip to positional encoding (for symmetry breaking)
...
#DIFFUSION-AWARE TRANSFORMER
def MyGT_gen(dataset_g):
    """
    Generate graph embeddings using Diffusion-Aware Transformer (DAT).
    
    This is the main function for graph encoding in the PIM architecture.
    It processes a graph and generates node/edge embeddings that capture
    structural and semantic information for downstream decision making.
    
    The function:
    1. Loads configuration parameters
    2. Adds positional encoding
    3. Trains/generates embeddings using Graph Transformer
    4. Returns embeddings for use by the decoder
    
    Args:
        dataset_g: Graph dataset object
