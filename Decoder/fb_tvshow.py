...
def experiment(
    ...
    device = variant.get('device', 'cuda:0')
    ...
    if dataset=='fb_tvshow':
        def read_fb_tvshow_data(file_path):
            edges = []
        ...
        file_path = 'data/fb_tvshow/fb_tvshow.SG'

        edges = read_fb_tvshow_data(file_path)
        G = nx.Graph()
        ...
        file_path = 'data/wiki/wiki-2.txt'

        edges = read_wiki_data(file_path)
        G = nx.Graph()
        G.add_edges_from(edges)
        G = from_networkx(G)
        num = G.num_nodes
    ...
    if env_name == 'IM':
        ...
        env_targets = [100000]  # evaluation conditioning targets
    ...
    # load dataset
    if dataset=='fb_tvshow':
        dataset_path = f'data/fb_tvshow/output-fb_tvshow-40traj-{m}-{int(budget*100)}.pkl'
        with open(dataset_path, 'rb') as f:
            trajectories = pickle.load(f)
    elif dataset == 'wiki':
        dataset_path = f'data/wiki/wiki2-{m}-{int(budget*100)}.pkl'
    elif dataset=='fb_food':
        dataset_path = f'data/fb_food/db_food-{m}-{int(budget*100)}nodes.pkl'
        with open(dataset_path, 'rb') as f:
            trajectories = pickle.load(f)
    ...
    print('=' * 50)
    print(f'Starting new experiment: {env_name} {dataset} {budget} {m}')
    ...
    def eval_episodes(target_rew):
        def fn(model):
            ...
            return {
                ...
                f'数据集为_{dataset}_传播式为_{m}_种子节点占比_{budget}_最大感染率_REWARD': influ1,
                f'数据集为_{dataset}_传播式为_{m}_种子节点占比_{budget}_最大感染率_R': influ2,
            }
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--dataset', type=str, default='fb_tvshow')  # medium, medium-replay, medium-expert, expert
    ...
    #种子节点占比
    # parser.add_argument('--budget', type=str, default='0.1')
    #传播方式
    # parser.add_argument('--method', type=str, default='IC')

    args = parser.parse_args()
    for dataset in ['fb_food']:
        for method in ['LT']:
            for budget in [0.05, 0.1, 0.2, 0.3, 0.4]:
                experiment(f'{dataset}_IM', variant=vars(args), dataset=dataset, method=method, budget=budget)