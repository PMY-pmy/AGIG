...
def experiment(
   ...
   #第二组对比数据
    ...
    with open('D:\\学习文件\\paper1\\code\\papercode1\\DeepIM\\DeepIM\\data\\power_grid_mean_IC10.SG', 'rb') as f:
        graph = pickle.load(f)
    ...
    num_node = adj.shape[0]
    data = []
    data1 = Data(num_nodes=num_node , edge_index=index, edge_attr=e)
    data.append(data1)
   ...
   #第三组简单图数据
    ...
    for i in range(len(data)):
        ...
        with open("D:\\学习文件\\paper1\\code\\papercode1\\MyGT\\MyGT\\configs\\configs1.json") as f:
            config = json.load(f)
        ...
        optimizer1 = optim.Adam(list(model.parameters())+list(model1.parameters()), lr=1e-4, weight_decay=1e-4)
            ...
            def forward(self, states, state_dim, actions, rewards, returns_to_go, timesteps, **kwargs):
                ...
                for i in range(1):
                    ...
                    #     state[int(i)] = 1
                        ...
                        # graph1.x = graph1.x.view(num_node,1)
                        # y1 = torch.where(graph1.x > 0, 1, 0)
                        # num_ones1 = (y1 == 1).sum().item()
                        graph1 = env.graph
                        graph1.x = state_pre
                        ...
                        y_true = torch.tensor(y_true, dtype=torch.float32, requires_grad=True,
                                              device=torch.device('cuda:0'))
                        loss = loss_inverse(h, y_true)
                        state_p=state_pre
                        state_p = torch.squeeze(state_p)
                        loss1 = loss_inverse(h, state_p)
                        optimizer1.zero_grad()
                        optimizer.zero_grad()
                        # loss.backward()
                        loss1.backward()
                        optimizer1.step()
                        optimizer.step()
                        sorted_tensor, sorted_indices = torch.sort(state_pre.squeeze(), descending=True)
                        # 提取前20个下标
                        n = int(0.05 * num_node)
                        top_n_indices_s = sorted_indices[:n]
                        top_n_indices_s_list.append(top_n_indices_s)
                        LOSS.append(loss1)
                        ...
                        #         graph1.features[i,0] = 1
                        #     graph1.features[:, 0][graph1.features[:, 0] != 1] = 0
                        #     print(graph1.features)
                        #     graph1.x = graph1.features[:, 0]
                        #     graph1.x =graph1.x.view(node_num1,1)
                        #     h, e, edge_index = self.model1.forward(graph1, lap_pos_enc)
                        #     y = torch.where(h > 0, 1, 0)
                        #     num_ones2 = (y == 1).sum().item()
                        #     influence1 = num_ones2 / len(y)
                        #     print(f'Number of elements equal to 1: {num_ones2}')
                        #     print(f'百分比为: {influence1}')
                        #
                        #     top10_indices = top10_indices.view(node_num_10)
                        ...
                        #     h, e, edge_index = self.model1.forward(graph1, lap_pos_enc)
                        #     y = torch.where(h > 0, 1, 0)
                        #     num_ones3 = (y == 1).sum().item()
                        #     influence2 = num_ones3 / len(y)
                        #     print(f'Number of elements equal to 1: {num_ones3}')
                        #     print(f'百分比为: {influence2}')
                        #
                        #     top5_indices = top5_indices.view(node_num_5)
                        ...
                        #     h, e, edge_index = self.model1.forward(graph1, lap_pos_enc)
                        #     y = torch.where(h > 0, 1, 0)
                        #     num_ones4 = (y == 1).sum().item()
                        #     influence3 = num_ones4 / len(y)
                        #     print(f'Number of elements equal to 1: {num_ones4}')
                        #     print(f'百分比为: {influence3}')
                        #
                        #     top1_indices = top1_indices.view(node_num_1)
                        ...
                        #     h, e, edge_index = self.model1.forward(graph1, lap_pos_enc)
                        #     y = torch.where(h > 0, 1, 0)
                        #     num_ones5 = (y == 1).sum().item()
                        #     influence4 = num_ones5 / len(y)
                        #     print(f'Number of elements equal to 1: {num_ones5}')
                        #     print(f'百分比为: {influence4}')
                print(state_pre,influence_list,top_n_indices_s_list)
                sorted_tensor, sorted_indices = torch.sort(state_pre.squeeze(), descending=True)
                # 提取前20个下标
                n=int(0.05*num_node)
                top_n_indices = sorted_indices[:n]
                print("Top 1% indices:", top_n_indices)
