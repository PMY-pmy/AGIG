...
class Environment:
    ...
    def reset(self, idx=None, training=True):
        ...
        self.state = [0 for _ in range(self.graph.num_nodes)]#最开始的节点状态都为0
        # IM
    ...
    def compute_reward(self, S):
        ...
        if self.use_cache and self.method == 'MC':
            S_str = f"{id(self.graph)}.{','.join(map(str, sorted(S)))}"#提取graph的标识符，然后将S中的元素提取出来并分别用逗号分隔
        ...
        if need_compute:
            if self.method == 'MC':
                ...
                es_inf = graph_utils.computeSIS(self.graph, S)
            elif self.method == 'IC':
                es_inf = graph_utils.computeIC(self.graph, S)
            elif self.method == 'LT':
                self.graph = to_networkx(self.graph)
                for v in self.graph.nodes():
                    self.graph.nodes[v]['threshold'] = random.uniform(0.1, 0.2)
                es_inf = graph_utils.computeLT(self.graph, S)
                self.graph = from_networkx(self.graph)
            else:
    ...
    def step(self, node, time_reward=None):
        ...
        if self.training:
            reward, R = self.compute_reward(S)
        else:
            if done:
